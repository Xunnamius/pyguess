...
class DisplayInterface():
    ...
    def MAR_exit(self, status=0):
        if(status == 0): self.gameMsg('Program Terminated.');
        else: self.errorMsg('Program Terminated Unexpectedly ('+str(status)+')!');
        self.evnMsg('(C) Copyright 2010 - Dark Gray. All Rights Reserved.');
        self.pause();
        os._exit(status);
    ...
    def menu(self, options, prompt="> ", initMsg="::Options::", errorMsg="Invalid Response.", prefix="- ", caseSense=False, capitalize=True, time_limit=None, limit_phrase=None):
        ...
        while options.has_key(input) == False:
            try:
                ...
                if(self.dieOnInterrupt):
                   choice = self.cooked_input("\n> Are you sure you wish to exit MAR? (y/n) ");
                   if(choice == 'y'): self.MAR_exit();
                   else: pass;
                else: self.MAR_exit();
